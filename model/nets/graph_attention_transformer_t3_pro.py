...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # Number of even scalar channels in the pairwise representation.
        # For the current EquiSite configuration this equals 128.
        self.num_scalar_channels = get_mul_0(self.irreps_pre_attn)
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst,
                edge_attr, edge_scalars, batch, **kwargs):
        ...
        # edge_attr[0]:
        # spherical harmonics of the global displacement vector
        #
        # edge_attr[1]:
        # invariant local geometry and relative sequence descriptors
        edge_scalar = edge_attr[1]

        if edge_scalar.size(-1) > self.num_scalar_channels:
            raise ValueError(
                'Invariant edge feature dimension '
                f'{edge_scalar.size(-1)} exceeds the available '
                f'{self.num_scalar_channels} scalar channels.'
            )

        # Current dimensions:
        # edge_scalar = 54 + 54 + 16 = 124
        # scalar capacity = 128
        edge_scalar = f.pad(
            edge_scalar,
            (
                0,
                self.num_scalar_channels - edge_scalar.size(-1),
            ),
            mode='constant',
            value=0.0,
        )

        # Inject invariant edge descriptors only into the 0e block.
        # The l=1 and l=2 components are not modified here.
        message = torch.cat(
            (
                message[:, :self.num_scalar_channels] + edge_scalar,
                message[:, self.num_scalar_channels:],
            ),
            dim=-1,
        )
        if self.nonlinear_message:          
            ...
            message = self.sep(message, edge_attr=edge_attr[0], edge_scalars=edge_scalars)
...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    def forward(self, f_in, pos, batch, node_atom, feature0, feature1, pos_emb, edge_index, **kwargs) -> torch.Tensor:
        ...
        feature0 = feature0.reshape(feature0.size(0), -1)
        feature1 = feature1.reshape(feature1.size(0), -1)
        pos_emb = pos_emb.reshape(pos_emb.size(0), -1)
        ...
        # edge_sh: equivariant directional features
        # edge_scalar: invariant scalar features
        edge_sh_list = [
            edge_sh,
            edge_scalar,
        ]
        ...
        for blk in self.blocks:
            node_features = blk(node_input=node_features, node_attr=node_attr, 
                edge_src=edge_src, edge_dst=edge_dst, edge_attr=edge_sh_list,
